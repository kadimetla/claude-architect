...
- `.mcp.json` - Segment 2 MCP config anchor (6 servers, 3 transports, env-var expansion). Two are course-owned and stdio: `document-mcp` points Claude Code at the course's own FastMCP demo (`examples/mcp_cli/mcp_server.py`), and `cca-study-mcp` points at the CCA Cert Buddy server (`cca-cert-buddy/mcp-server/index.ts`).
